...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope notion-agent
...
scope:** the default personal scope is **private** (`--scope notion-agent
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `notion-agent`). Private records are served only to a recall in the same
