...
notion-agent vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. notion-agent
...
#   - consumer-identifying prose: `guildmaster` → `notion-agent` (NOT
...
If a re-sync would lose a notion-agent adaptation, lift the change
...
#  `which notion-agent vendors from guildmaster`).
