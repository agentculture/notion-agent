"""Unified CLI entry point for notion-agent.
...
--------------------------
Every handler raises :class:`notion_agent.cli._errors.CliError` on
failure; ``main()`` catches it via :func:`_dispatch` and routes through
:mod:`notion_agent.cli._output`. Unknown exceptions are wrapped into a
...
from notion_agent.cli._output import emit_error

_ISSUES_URL = "https://github.com/agentculture/notion-agent/issues"
