"""``notion-agent cli`` — noun grouping CLI-surface introspection.
...
from notion_agent.cli._commands.overview import cli_sections, emit_overview
...
def _no_verb(args: argparse.Namespace) -> int:
    # `notion-agent cli` with no sub-verb prints the noun's overview.
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "cli",
        help="CLI-surface introspection (see 'notion-agent cli overview').",
    )
    ...
    ov = noun_sub.add_parser("overview", help="Describe the notion-agent CLI surface.")
